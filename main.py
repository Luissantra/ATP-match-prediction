...
if __name__ == "__main__":
    ...
    # 3. Entrenar modelo único (LogReg) + calibrar (sigmoid, CV temporal purgado)
    print("\n[3/5] Entrenando LogReg con GridSearchCV + CV temporal con embargo...")
    dates_train = df_train['tourney_date'].values
    X_train_arr = X_train.values
    y_train_arr = y_train.values

    modelo_base, cv_log_loss, best_params = entrenar_modelo(X_train_arr, y_train_arr, dates=dates_train)
    print(f"  best_params={best_params}  cv_log_loss={cv_log_loss:.4f}")
    modelo = calibrar_modelo(modelo_base, X_train_arr, y_train_arr, dates=dates_train)

    # Explicabilidad: coeficientes / odds-ratio del modelo base
    coefs = coeficientes_modelo(modelo_base, FEATURES)
    print("\n  Coeficientes (log-odds por +1 std · odds-ratio):")
    for feat, v in coefs.items():
    ...
    print("\n[4b/5] Baseline ELO-híbrido vs ML (¿aporta el modelo sobre la resta de ELO?)...")
    ...
    if len(y_sec) > 0:
        ...
        met_sec = evaluar_con_ic(modelo, X_sec, y_sec)
    ...
    # modelos_atp.pkl: el modelo único calibrado (LogReg)
    with open('models/modelos_atp.pkl', 'wb') as f:
        pickle.dump(modelo, f)

    # metrics_atp.pkl: métricas test principal (2025)
    ...
    with open('models/stats_jugadores.pkl', 'wb') as f:
        pickle.dump({'elo_general': ratings_finales, 'elo_superficie': ratings_superficie,
                     'stats': stats_jugadores, 'coeficientes': coefs,
                     'sklearn_version': sklearn.__version__}, f)

    print(f"  modelos_atp.pkl      — LogReg calibrado (entrenado 2020-{TRAIN_END_YEAR-1})")
    print(f"  metrics_atp.pkl      — métricas test {TEST_YEAR}")
    print("  stats_jugadores.pkl  — ELO/rank/age + coeficientes")
